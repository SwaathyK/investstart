...
export default function RealMoneyPage() {
  ...
  useEffect(() => {
    const checkLogin = () => {
      ...
      const savedBalance = localStorage.getItem('brokee_real_balance')
      if (savedBalance) {
        setRealBalance(parseFloat(savedBalance))
      } else {
      ...
      // Load real money portfolio
      const savedPortfolio = localStorage.getItem('brokee_real_portfolio')
      if (savedPortfolio) {
        setPortfolio(JSON.parse(savedPortfolio))
      }

      // Load real money transactions
      const savedTransactions = localStorage.getItem('brokee_real_transactions')
      if (savedTransactions) {
        const parsed = JSON.parse(savedTransactions)
        setTransactions(parsed.map((t: any) => ({ ...t, timestamp: new Date(t.timestamp) })))
      }

      // Load points and trades count for real money
      const savedPoints = localStorage.getItem('brokee_real_trading_points')
      if (savedPoints) {
        setPoints(parseInt(savedPoints))
      }

      const savedTrades = localStorage.getItem('brokee_real_trades_count')
      if (savedTrades) {
  ...
  const handleBuy = () => {
    ...
    const confirmed = window.confirm(
      ...
      `You are about to buy ${sharesNum} shares of ${selectedStock.symbol} for £${totalCost.toFixed(2)}.\n\n` +
    ...
    localStorage.setItem('brokee_real_balance', newBalance.toFixed(2))
    ...
    localStorage.setItem('brokee_real_portfolio', JSON.stringify(newPortfolio))
    ...
    localStorage.setItem('brokee_real_transactions', JSON.stringify(newTransactions))
    ...
    localStorage.setItem('brokee_real_trading_points', newPoints.toString())
    localStorage.setItem('brokee_real_trades_count', newTradesCount.toString())
  ...
  const handleSell = () => {
    ...
    const confirmed = window.confirm(
      `⚠️ REAL MONEY TRANSACTION ⚠️\n\n` +
      `You are about to sell ${sharesNum} shares of ${selectedStock.symbol} for £${totalValue.toFixed(2)}.\n\n` +
      `This will add REAL MONEY to your account.\n\n` +
      `Do you want to proceed?`
    )
    ...
    // Update balance
    const newBalance = realBalance + totalValue
    setRealBalance(newBalance)
    localStorage.setItem('brokee_real_balance', newBalance.toFixed(2))
  ...
  const handlePortfolioSell = () => {
    ...
    const confirmed = window.confirm(
      `⚠️ REAL MONEY TRANSACTION ⚠️\n\n` +
      `You are about to sell ${sharesNum} shares of ${selectedPortfolioItem.symbol} for £${totalValue.toFixed(2)}.\n\n` +
      `This will add REAL MONEY to your account.\n\n` +
      `Do you want to proceed?`
    )
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-6 bg-red-50 border-2 border-red-200 rounded-xl p-4">
          <div className="flex items-start space-x-3">
            ...
            <div>
              <h3 className="font-bold text-red-900 mb-1">⚠️ Real Money Trading</h3>
              <p className="text-sm text-red-800">
                You are trading with REAL MONEY. All transactions are final and will affect your actual account balance. 
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-bold text-gray-900 mb-2">Real Money Trading</h1>
              <p className="text-gray-600">Invest with real money - Trade carefully!</p>
            </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-6">
          <div className="bg-white rounded-xl shadow-lg p-6 border-2 border-primary-200">
            ...
            <p className="text-3xl font-bold text-gray-900">£{realBalance.toFixed(2)}</p>
            <p className="text-sm text-gray-500 mt-1">Real Money Balance</p>
            {realBalance === 0 && (
              <div className="mt-3 p-2 bg-yellow-50 rounded-lg border border-yellow-200">
                <p className="text-xs text-yellow-800">
                  💡 Deposit funds to start trading with real money
                </p>
              </div>
            )}
          </div>
        ...
        {/* Tab Content - Same as simulation page but with real money context */}
        {activeTab === 'market' && (
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-1">
                          ...
                          <button
                            onClick={(e) => {
                              e.stopPropagation()
                              const saved = localStorage.getItem('brokee_real_watchlist')
                              ...
                              localStorage.setItem('brokee_real_watchlist', JSON.stringify(newWatchlist))
                            }}
                            className="ml-auto"
                          >
                    ...
                    <div className="bg-gray-50 rounded-lg p-4">
                      ...
                      {action === 'buy' && (
                        <div className="mt-2 text-sm text-gray-500">
                          Balance after: £{(realBalance - parseFloat(shares) * selectedStock.price).toFixed(2)}
                        </div>
                      )}
                      {action === 'sell' && (
                        <div className="mt-2 text-sm text-gray-500">
        ...
        {/* Portfolio Tab - Same structure as simulation */}
        {activeTab === 'portfolio' && (
          <div className="space-y-6">
            {/* Portfolio */}
            <div className="bg-white rounded-xl shadow-lg p-6">
